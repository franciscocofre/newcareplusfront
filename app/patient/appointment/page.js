...
export default function PatientAppointmentPage() {
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:3001/api/users/professionals/specialty?specialty=${selectedSpecialty}`,
  ...
  const fetchSchedules = async (professional_id) => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:3001/api/schedules/${professional_id}/available-schedules`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const confirmAppointment = async (e) => {
    ...
    try {
      ...
      const paymentResponse = await axios.post(
        "http://localhost:3001/api/payments/create-payment-link",
        { appointment_id: id },
        { headers: { Authorization: `Bearer ${token}` } }
      );
